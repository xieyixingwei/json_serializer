...
class MirrorContext extends RuntimeContext {
  ...
  List<Compiler> get compilers {
    //final libs = currentMirrorSystem().libraries;
    //libs.forEach((key, value) => print("--- ${key.path}"));
    return types
...
T? firstMetadataOfType<T>(DeclarationMirror dm, {TypeMirror? dynamicType}) {
  ...
  final found = dm.metadata.firstWhereOrNull((im) => im.type.isSubtypeOf(tMirror));
  return found == null ? null : found.reflectee as T;
}
